...
#if defined(NDEBUG)
# error "PearlBitcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe95f47fc6fa4c751e413906412363f38a3a61d1c86b3d522d2afd9bbe7e3dada");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PearlBitcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PearlBitcoin Signed Message:\n";
...
{
    // PearlBitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PearlBitcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PearlBitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1512904520, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "25121989";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 250 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04dc88f3f43358e4abdeb4289e2f7f77570cd81d7c19bdd0b4ad0d105c7ecd5ae4d0118626187efd4c1f4fc46782af6d1adbebd63a596504a8e64656e7454aaa48") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1512904520;
        ...
        assert(block.hashMerkleRoot == uint256("0xb69e620998ced0eedbbaf47723d23ac7409c69d8b4831fe497101ff39338e410"));
...
unsigned char pchMessageStart[4] = { 0xb6, 0x88, 0x51, 0x22 }; // PearlBitcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PearlBitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PearlBitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PearlBitcoinMiner(CWallet *pwallet)
{
    printf("PearlBitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pearlbitcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PearlBitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PearlBitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PearlBitcoinMiner, pwallet));
}
